...
const EV = {
  ...
  SKIP_TO_VOTE: "game:skipToVote", VOTE_CAST: "vote:cast", NEXT_ROUND: "game:nextRound",
...
async function scenarioA() {
  ...
  const { players, socks, roleOf, latest, w } = await setup(3, { spyCount: 1, blindCount: 0 });
  const spyIdx = [0, 1, 2].find((i) => roleOf(i) === "SPY");
  check("đúng 1 gián điệp", [0, 1, 2].filter((i) => roleOf(i) === "SPY").length === 1);
  reveal(socks);
...
async function scenarioB() {
  ...
  const { players, socks, roleOf, latest, w } = await setup(3, { spyCount: 1, blindCount: 0 });
  const civIdx = [0, 1, 2].find((i) => roleOf(i) === "CIVILIAN");
  reveal(socks);
  await w((s) => s.phase === "DISCUSS", "discuss");
  socks[0].emit(EV.SKIP_TO_VOTE);
  await w((s) => s.phase === "VOTE", "vote");
...
async function scenarioC() {
  console.log("\n▶ C) Phân vai có Người Mù");
  const { socks, roleOf, wordOf } = await setup(5, { spyCount: 1, blindCount: 1 });
  const idx = [0, 1, 2, 3, 4];
  const spies = idx.filter((i) => roleOf(i) === "SPY");
  const blinds = idx.filter((i) => roleOf(i) === "BLIND");
  const civs = idx.filter((i) => roleOf(i) === "CIVILIAN");
  check("đúng 1 gián điệp", spies.length === 1, `(${spies.length})`);
  check("đúng 1 người mù", blinds.length === 1, `(${blinds.length})`);
  check("đúng 3 dân thường", civs.length === 3, `(${civs.length})`);
  check("người mù không có từ khóa", wordOf(blinds[0]) == null);
...
async function scenarioE() {
  ...
  const { players, socks, latest, w } = await setup(4, { spyCount: 1, blindCount: 0 });
  reveal(socks);
  await w((s) => s.phase === "DISCUSS", "discuss");
  socks[0].emit(EV.SKIP_TO_VOTE);
  await w((s) => s.phase === "VOTE", "vote");
...
async function scenarioF() {
  ...
  const { players, socks, roleOf, latest, w, alive } = await setup(5, { spyCount: 1, blindCount: 0 });
  const i5 = [0, 1, 2, 3, 4];
  const spyIdx = i5.find((i) => roleOf(i) === "SPY");
  const civIdx = i5.find((i) => roleOf(i) === "CIVILIAN");
  reveal(socks);
